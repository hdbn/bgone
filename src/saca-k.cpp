...
void induceSAl1(int *SA, int *s, 
  ...
  for(i=1; i<n; i++) {
    j=SA[i];
    if(j<0 && j!=EMP) { // is SA[i] a counter?
      for(h=0; h<-j; h++)
        SA[i+h]=SA[i+h+1];
      SA[i+h]=EMP;
    }
...
void getSAlms(unsigned int *SA, 
  ...
  for(i=n-2; i>0; i--) {
    ...
    // if (j == 0) std::cout << "level=" << level <<std::endl;
    // assert(j > 0);
    // assert(j < n);
  ...
  // init SA[n1..n-1]
  for(i=n1; i<n; i++) SA[i]=level?EMP:0; 
}
